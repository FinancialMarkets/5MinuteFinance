...
-  The app may take 5 seconds or so to load.  Hit backspace and start typing in a company's name.  The app will search for the company as you type.
...
We do have various methods to predict risk, such as the [ARCH/GARCH](https://en.wikipedia.org/wiki/Autoregressive_conditional_heteroskedasticity) class of models and predictions from [Kalman Filtering](https://en.wikipedia.org/wiki/Kalman_filter) and [Markov Switching](http://complete-markets.com/2014/02/markov-regime-switching-volatility-in-natural-gas-futures-ngh4/) algorithms. In these models, however, future risk is a function of past risk, and so in low-risk periods the models will predict low risk in the future, and for high-risk periods, they will predict high risk in the future.  This makes sense, because we know [volatility clusters](https://en.wikipedia.org/wiki/Volatility_clustering).  
...
-  This method works well when there are known events -- such as an option earnings season.  However, it will fail if the events are unforeseen, such as the BP Gulf of Mexico oil spill.
...
Click the following links to see the [code](https://github.com/FinancialMarkets/5MinuteFinance/blob/master/Risk/risk_future_past/risk_over_time.Rmd), [authors of this presentation](https://github.com/FinancialMarkets/5MinuteFinance/blame/master/Risk/risk_future_past/risk_over_time.Rmd), and [all the collaborators who have contributed to 5MinuteFinance](https://github.com/FinancialMarkets/5MinuteFinance/graphs/contributors).
