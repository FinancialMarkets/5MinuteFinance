...
-  This is important, because we simply cannot know what the stock's return will be.

-  The idea is that we don't need the expected return because we are going to hedge our risk from the stock's returns (whatever they are).
...
- However it is much more convenient to simply use put-call parity, which is another topic altogether.
