...
sidebarLayout(
    sidebarPanel(
        ...
        numericInput("Stock", "Present Stock Price", min = .01, max = 2000, value = 50, step = .05),
        numericInput("Vol", "Annualized Volatility", min = .01, max = 20, value = 0.20, step = .01),
        numericInput("Strike", "Strike Price", min = .01, max = 1, value = 52, step = .05),
        numericInput("Rf", "Risk Free Rate", min = 0.001, max = 0.5, value = 0.01, step = .001),
        numericInput("Time", "Years to Expiration", min = 0.01, max = 50, value = 1, step = .01),
        numericInput("Div", "Dividend Yield", min = 0, max = 1, value = 0, step = .01),
        radioButtons("out", "Value to See", choices = list("Call Price" = 1, "d1" = 2, "d2" = 3), selected = 1) 
                  ),
    mainPanel(
        ...
        renderPlot({
            ...
            if (input$out == 1){
                ...
                text(x = 0.5, y = 0.5, labels = paste(round(C, 2)), cex = 5)
...
The present stock price is easily observable, and the exercise price and time to maturity are aspect of the option contract. The parameters which are less easily observed are:
...
We can extract a volatility estimate from traded options by plugging the option price into the Black-Scholes formula, and solving for volatility. This volatility estimate is call the options 'Implied Volatility'.
...
sidebarLayout(
    sidebarPanel(
        ...
        numericInput("Stock1", "Present Stock Price", min = .01, max = 2000, value = 50, step = .05),
        numericInput("Option1", "Call Option Premium (Price)", min = .01, max = 20, value = 2, step = .01),
        numericInput("Strike1", "Strike Price", min = .01, max = 1, value = 52, step = .05),
        numericInput("Rf1", "Risk Free Rate", min = 0.001, max = 0.5, value = 0.01, step = .001),
        numericInput("Time1", "Years to Expiration", min = 0.01, max = 50, value = 1, step = .01),
        numericInput("Div1", "Dividend Yield", min = 0, max = 1, value = 0, step = .01)
                  ),
    mainPanel(
...
- However is is much more convenient to simply use [put-call parity](http://127.0.0.1), which we cover in the linked presentation.
