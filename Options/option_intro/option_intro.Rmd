...
An American option's value (both call and put) can never be less than the option's intrinsic.  Why not?
...
>+ In the next slide you can graph a European Call option's (on a non-dividend paying stock) intrinsic, premium, and time value for any set of inputs. 
...
sidebarLayout(
    sidebarPanel(
        ...
        div(style="height: 75px;", sliderInput("maturity", label = "Years to Maturity", max = 2, min = .1, value = .5, step = .1, ticks = FALSE)),
        div(style="height: 75px;", sliderInput("riskFree", label = "Risk Free Rate", max = .2, min = .01, value = .02, step = .01, ticks = FALSE)),
        div(style="height: 75px;", sliderInput("divYield", label = "Dividend Yield", max = .2, min = .01, value = .02, step = .01, ticks = FALSE))
    ...
    mainPanel(
        renderPlot({
            ...
            # if(input$type == "1") {
                ...
                plot(intrinsic, main = "Call Option Premium and Intrinsic Value", ylab = "$", xlab = "Underlying Stock Price ($)", type = "l", lwd = 5, sub = "Black-Scholes value for a European call on a non-dividend paying stock")
                                        #             axis(1, at = 1:20, labels = yAxis)
                lines(optionValue, lwd = 5, col = "dodgerblue4")
                                        #           lines(1:20, durLine, lwd = 5, col = 3)
                legend('topleft', c("Intrinsic Value", "Option Value", "Time Value"), lty = c(1,1,1), col=c(1,"dodgerblue4", "lightblue"), lwd = c(5,5, 15))
                ##  shade region  ----
                polygon(c(1:100, rev(1:100)), c(intrinsic, rev(optionValue)), col = "lightblue", border = NA)
...
sidebarLayout(
    sidebarPanel(
        sliderInput("strike.p", label = "Strike Price", max = 100, min = 0, value = 50, step = 1),
        sliderInput("volatility.p", label = "Return Volatility", max = .5, min = 0.01, value = .20, step = .01),
        sliderInput("maturity.p", label = "Years to Maturity", max = 2, min = .1, value = .5, step = .1),
        sliderInput("riskFree.p", label = "Risk Free Rate", max = .2, min = .01, value = .02, step = .01)
        # radioButtons("type", label = "Option Type", choices = c("Call" = "1", "Put" = "2"), selected = "Call")
        ),
                                        #     tabsetPanel(
                                        #                 tabPanel(title = "Option",
    mainPanel(
        renderPlot({
            ## calculate intrinsic value
            intrinsic <- 1:100 - input$strike.p
            intrinsic[intrinsic < 0] <- 0

            ## calculate time value by Black-Scholes for all integer stock prices from 1 to 100 ----
            d1 <- (log(1:100/input$strike.p) + (input$riskFree.p + (input$volatility.p^2)/2) * input$maturity.p) / (input$volatility.p * sqrt(input$maturity.p))
            d2 <- d1 - input$volatility.p * sqrt(input$maturity.p)
            optionValue <- 1:100 * pnorm(d1) - input$strike.p * exp(-input$riskFree.p * input$maturity.p) * pnorm(d2)
            timeValue <- optionValue - intrinsic

            ## calculate put value by put-call parity
            putValue <- optionValue + input$strike.p * exp(-input$riskFree.p * input$maturity.p) - 1:100

            ## calculate put intrinsic value ----
            intrinsicPut <- input$strike.p - 1:100
            intrinsicPut[intrinsicPut < 0] <- 0
