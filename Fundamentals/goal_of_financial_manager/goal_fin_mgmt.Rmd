...
inputPanel(
 sliderInput("risk", "Risk (Leverage)", min = 0, max = 100, step = 1, value = 20)
)

renderPlot({

  rate <- 0.05 + sqrt( ( (input$risk - 60) / 50) ^ 2 + .02 )
  CFs <- 10 * sqrt( (input$risk / 60) )
  Value <- CFs / rate
  ...
  text(x = 0.2, y = 0.5,
       labels = paste0("Stock price = ", round( Value, 2), "; Cash Flows = ",
                       round( CFs, 2 ), "; Req. Return = ", round( rate, 2 )),
       cex = 4)
