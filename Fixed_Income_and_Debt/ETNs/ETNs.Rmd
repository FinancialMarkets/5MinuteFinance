...
ETNs issue and redeem notes throughout the trading day to keep the ETN price in line with the value of the underlying.  

<div class="MIfooter"><img src="mi.png" style="height:50px;"></div> 

## ETNs and Exotic Underlyings

Because an ETN is a promise to pay an underlying's return, and the underlying doesn't actually have to be purchased, the ETN can track returns that ETFs may have a hard time replicating.  For example consider:
...
-  The [iPath® US Treasury Flattener ETN](http://www.ipathetn.com/US/16/en/details.app?instrumentId=259102) which *inversely* tracks the Barclays US Treasury 2Y/10Y Yield Curve Index.  
...
-  Note that there are substantially fewer queries for ETN relative to ETF.  This is not surprising given ETFs have been around longer, and contain thelarge index funds.
...
While it is possible for ETNs to have no tracking error, that is not always the case. On the next slide is the ETN and underlying index cumulative returns for a random sample of iPath ETNs.  

-  While most do a good job of tracking the index, a few (DTYL, DTYS, and PGD) do not.

-  This is because inverse and leveraged ETNs are meant to meet thier performanace objective (relative to an underlying index) on a daily basis.  Matching the index over a longer period is not possible given the leverage used in the ETN---which means returns will compound at a different rate between the index and the ETN.  
