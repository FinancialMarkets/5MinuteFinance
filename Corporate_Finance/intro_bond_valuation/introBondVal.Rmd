---
title: "Intro to Bond Valuation [very preliminary]"
...
Given the previous information we know the cash flow structure and the rate at which to discount the cash flows.  For example, say a bond has a 5% coupon rate, a \$1000 par value, has 10 years to maturity, payments are annual, and lastly has a 8% $YTM$.
...
- The present value of these cash flows at the $YTM$ (the value of the bond $B_0$) is:
...
- This is often calculated using the present value of an ordinary annuity formula: $B_0 = \$50\left(\frac{1-\frac{1}{1.08^{10}}}{.08}\right) + \frac{\$1000}{1.08^{10}} = \$798.69$

- The two calculations are equivalent.  The former is more popular if used in a spreadsheet.  The latter was mainly taught because people were doing these calculations by calculator.
...
One misconception about bonds is that if you buy a bond with a 9\% $YTM$, and hold the bond to maturity, that so long as the bond doesn't default you will earn 9\% per year.
...
## A Note on $YTM$ [remove?]

To get the value of a bond we discount each cash flow at the $YTM$.  However to get the $YTM$ we set the discounted cash flows equal to the market price, and solve for the discount rate ($YTM$).  This, of course, is circular.
...
- Two ten year US Treasury notes will have the same $YTM$, but can have vastly different prices. A 10 year note issued yesterday will trade near \$1000 because its $YTM$ is very close to its coupon.  A 30-year Treasury bond issued 20 years ago (at a say 18\% coupon) is now a 10 year Treasury note with a price based on its $YTM$ and coupon rate differential. The bond's will have different prices, but the same $YTM$.      

<div class="MIfooter"><img src="mi.png" style="height:50px;"></div> 

## Semiannual vs Annual Values

-  If the bond is trading at a discount ($YTM$ above the coupon rate), then switching from annual to semiannual wil lower the value of the bond.
...
Click [here](https://github.com/FinancialMarkets/5MinuteFinance/blob/master/Corporate_Finance/intro_bond_valuation/introBondVal.Rmd) to see the code and collaborators for this presentation
