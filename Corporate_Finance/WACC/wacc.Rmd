...
-  If we are considering investing in a project that is about as risky as the overall firm, we can use the WACC as the discount rate in our [NPV](https://micfm.shinyapps.io/NPV_and_IRR_mobile) calculation.
...
-  The benefits of this approach are that it can be applied to any stock (any capital asset), and it explicitly takes into account risk. 
...
The cost of debt is yield required by the market on the firm's debt. If the debt is actively traded (meaning we can trust the price), then it is the bond's yield-to-maturity.  If no debt is actively traded, you can infer the cost of debt from the outstanding debt's credit rating.
...
-  We thus adjust the cost of debt by the amount of the tax shield, $y_{at} = y_{bt}(1 - \tau)$ where $y_{at}$ is the yield after tax, $y_{bt}$ is the yield before tax, and $\tau$ is the tax rate.  $y_{at}$ is the cost of debt.
