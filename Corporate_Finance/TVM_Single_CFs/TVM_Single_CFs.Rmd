...
In this presentation we'll cover the mechanics of moving money from one point in time to another.

-  Only once cash flows have all been moved to the same time period, they can be added or subtracted.  
...
-  You can also use the app to see the effect of small differences in interest rates on the future value over many years.  For example, the future value of a dollar is worth 33\% more of invested for 30 years at 5\% instead of 4\%.
...
renderPlot({
  ...
  legend("topleft", "Future Value of $1", col = 3, lwd = 3, lty = 1, cex = 1.5)
})
#}}}
```
...
renderPlot({
  ...
  legend("bottomleft", "Present Value of $1", col = 3, lwd = 3, lty = 1, cex = 1.5)
})
#}}}
```
