...
max.rands <- 100000
inputPanel(
  sliderInput(inputId = "rands", label = "Number of Random Numbers in Monte Carlo", min = 50, max = max.rands, step = 1000, value = 50, animate = TRUE)
...
renderPlot({ 
  ...
  hist(call[1:input$rands], main = paste0("Call Premium $", round(mean(call[1:input$rands]), 2)), xlab = "Call Option Value", breaks = 100, freq = FALSE)
  legend('topright', c(paste0("Black-Scholes Value $", round(call.bs, 2)), paste0("Difference $", round(abs(call.bs - mean(call[1:input$rands])), 2))))
})
```
